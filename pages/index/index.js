...
Page({
  ...
  fetchData(){
    ...
    wx.request({url:API+'/races',success:(res)=>{
      ...
      const now=new Date();const next=races.find(r=>new Date(r.date)>now)||races[races.length-1]
      const last=races.filter(r=>new Date(r.date)<=now).pop()
      this.setData({nextRace:next||null,lastRace:last||null,seasonPct:next?Math.round(next.round/24*100):0,loading:false})
    ...
    },fail:()=>{this.setData({loading:false,error:true});wx.stopPullDownRefresh()}})
  },
