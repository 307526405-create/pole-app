...
Page({
  ...
  onLoad() {
    wx.request({
      ...
      success: (res) => {
        const now = new Date()
        this.setData({
          races: (res.data.data || []).map(r => ({
            round: r.round,
            flag: FLAGS[r.country] || '🏁',
            gp: RACE_CN[r.name] || r.name,
      ...
      success: (res) => {
        ...
        ds[0] = (res.data.data || []).map(d => {
          ...
          return {
            ...
            name: t[0],
            en: t[1],
            team: (T_CN[d.constructor] || [d.constructor])[0],
            pts: d.points,
            img: t[2]
          }
        })
  ...
  switchSub(e) {
    this.setData({ subtab: parseInt(e.currentTarget.dataset.idx) })
  }
})
